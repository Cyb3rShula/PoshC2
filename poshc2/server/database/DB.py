...
def new_urldetails(Name, URL, HostHeader, ProxyURL, ProxyUsername, ProxyPassword, CredentialExpiry):
    c = get_conn().cursor()
    command = convert_query("INSERT INTO URLs (Name, URL, HostHeader, ProxyURL, ProxyUsername, ProxyPassword, CredentialExpiry) VALUES (?, ?, ?, ?, ?, ?, ?)")
...
def new_implant(RandomURI, URLID, User, Hostname, IpAddress, Key, FirstSeen, LastSeen, PID, Arch, Domain, Alive, Sleep, ModsLoaded, Pivot, Label):
    c = get_conn().cursor()
    command = convert_query("INSERT INTO Implants (RandomURI, URLID, \"User\", Hostname, IpAddress, Key, FirstSeen, LastSeen, PID, Arch, Domain, Alive, Sleep, ModsLoaded, Pivot, Label) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
...
def insert_task(randomuri, command, user):
    ...
    if user is None:
        user = ""
    query = convert_query("INSERT INTO Tasks (RandomURI, Command, Output, \"User\", SentTime, CompletedTime, ImplantID) VALUES (?, ?, ?, ?, ?, ?, ?)")
...
def update_item(column, table, value, wherecolumn=None, where=None):
    ...
    if wherecolumn is None:
        ...
        query = f"UPDATE {table} SET {column}=? WHERE {wherecolumn}=?" % (table, column, wherecolumn)
...
def insert_cred(domain, username, password, hash):
    if check_if_cred_exists(domain, username, password, hash):
        return None
    c = get_conn().cursor()
    command = convert_query("INSERT INTO Creds (Domain, Username, Password, Hash) VALUES (?, ?, ?, ?)")
...
def new_c2_message(message):
    ...
    c = get_conn().cursor()
    command = convert_query("INSERT INTO C2_Messages (Message,Read) VALUES (?,'No')")
